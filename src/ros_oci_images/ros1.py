...
def build_images(
    *, registry, name, rosdistro, base_image, architectures, dry_run=False
    ...
    build_one_package_image(
        f"ros-{rosdistro}-robot",
        registry=registry,
        name=name,
        tag=f"{rosdistro}-robot",
        base_image=ros_base,
        architectures=architectures,
        dry_run=dry_run,
    )
    ...
    for arch in architectures:
        ...
        b.run(
            [
                "bash",
                "-c",
                'echo "deb [ signed-by=/usr/share/keyrings/ros1-latest-archive-keyring.gpg ] http://packages.ros.org/ros/ubuntu `lsb_release -sc` main" > /etc/apt/sources.list.d/ros1-latest.list',
            ]
        )
